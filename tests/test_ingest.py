...
from src.config import SessionPaths
...
class IngestTests(unittest.TestCase):
    ...
    def test_collector_writes_laps_and_enriched_metadata(self) -> None:
        temp_root = Path(tempfile.mkdtemp())
        collector = datacollector(ip="127.0.0.1", port=0, session_id="session_test_ingest")
        ...
        try:
            packet_lap_1 = self._build_packet(timestamp_ms=1000, lap_number=1, position_x=0.0, speed=30.0)
            packet_lap_1_b = self._build_packet(timestamp_ms=1050, lap_number=1, position_x=2.0, speed=31.0)
            packet_lap_2 = self._build_packet(timestamp_ms=1100, lap_number=2, position_x=4.0, speed=28.0)
    ...
    @staticmethod
    def _build_packet(timestamp_ms: int, lap_number: int, position_x: float, speed: float) -> bytes:
        decoder = ForzaTelemetryDecoder()
        values = [0] * 90
        values[0] = 1
