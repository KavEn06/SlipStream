...
def evaluate_lap_validation(
    ...
    end_progress_m = _resolve_end_progress_m(raw_df, processed_df, final_cumulative_distance_m)
...
def _resolve_end_progress_m(
    raw_df: pd.DataFrame,
    processed_df: pd.DataFrame,
    final_cumulative_distance_m: float,
) -> float:
    if "DistanceTraveledM" in processed_df.columns:
        processed_distance = pd.to_numeric(processed_df["DistanceTraveledM"], errors="coerce")
        max_processed_distance = _max_numeric_value(processed_distance)
        if max_processed_distance is not None:
            return max_processed_distance

    raw_distance = pd.to_numeric(raw_df["DistanceTraveled"], errors="coerce")
    max_raw_distance = _max_numeric_value(raw_distance)
    if max_raw_distance is not None:
        return max_raw_distance
